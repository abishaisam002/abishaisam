...
const HERO_SOCIAL_LABELS = ['LinkedIn', 'Email', 'Phone']
...
const VID_FRAG = `
  ...
  void main() {
    ...
    if (uCanvasAspect > uVideoAspect) {
      ...
      if (uv.x < 0.0 || uv.x > 1.0) outOfBounds = true;
    } else {
      float s = uVideoAspect / uCanvasAspect;
      uv.y = (vUv.y - 0.5) * s + 0.5;
      if (uv.y < 0.0 || uv.y > 1.0) outOfBounds = true;
    }
    if (outOfBounds) {
...
export default function PublicationsFooterSection() {
  ...
  const categories = {
    Education: PUBS.filter(item => item.type === 'Education'),
    Certifications: PUBS.filter(item => item.type === 'Certification'),
    Skills: PUBS.filter(item => ['Skills', 'AI Tech', 'Software'].includes(item.type)),
    Languages: PUBS.filter(item => item.type === 'Languages'),
  }
    ...
    <div ref={wrapperRef} className={styles.wrapper}>
      <div ref={stickyRef} className={styles.sticky}>
        ...
        <div ref={pubContentRef} className={styles.pubContent}>
          <span className={styles.watermark} aria-hidden>EXPERTISE</span>

          <div className={styles.pubHero}>
            <p  ref={labelRef}   className={styles.label}>Credentials &amp; Core Skills</p>
            <h2 ref={headingRef} className={styles.heading}>Expertise</h2>
          </div>
          ...
          <div className={styles.gridContainer}>
            {Object.entries(categories).map(([catName, items], catIdx) => (
              <div
                key={catName}
                ref={el => { itemRefs.current[catIdx] = el }}
                  ...
                  {items.map((item) => (
                    <div key={item.id} className={styles.subItem}>
                      <div className={styles.subItemHeader}>
                        <span className={styles.subItemTitle}>{item.title}</span>
                        {item.year && <span className={styles.subItemYear}>{item.year}</span>}
                      </div>
                      <span className={styles.subItemSubtitle}>{item.subtitle}</span>
          ...
          <div ref={bottomBarRef} className={styles.bottomBar}>
            <div className={styles.bottomLeft}>
              <div className={styles.monogram}>
                <span className={styles.monoLetters}>AS</span>
